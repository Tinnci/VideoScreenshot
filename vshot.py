...
# 显式导出runAllImageSimilaryFun函数，便于在其他模块导入
__all__ = ["comp", "get_method_name", "runAllImageSimilaryFun"]


class Logger:
    """日志管理类，用于控制不同级别的日志输出"""

    def __init__(self, quiet=False, verbose_mode=False):
        self.quiet = quiet
        self.verbose_mode = verbose_mode  # 重命名属性以避免与方法名冲突
    ...
    def verbose(self, message, end="\n"):
        """仅在详细模式下输出信息"""
        if self.verbose_mode:  # 使用重命名后的属性
            print(message, end=end)
    ...
    def update_progress(
        self, progress, total, current, prefix="", suffix="", bar_length=30, min_update_interval=0.5
    ):
        """更新进度条，避免频繁刷新"""
        ...
        # 完整进度信息
        progress_text = f"\r{prefix}{progress_bar} | {current}/{total}{eta_str} {suffix}"
...
def fargv():
    ...
    parser.add_argument("VideoFilePath", type=str, help=("输入文件, filepath"))
    parser.add_argument(
        "-o", "--outdir", type=str, default="./out/", help=("输出文件夹路径, dirpath")
    )
    parser.add_argument(
        "-S",
        "--Similarity",
    ...
    parser.add_argument("-s", "--spend", type=int, default=5, help=("间隔帧数, int"))
    parser.add_argument(
        "-m",
        "--method",
        type=int,
    ...
    parser.add_argument(
        "--all", action="store_true", default=False, help="是否使用让5种算法都进行计算"
    )
    parser.add_argument(
        "-q", "--quiet", action="store_true", default=False, help="启用安静模式，减少输出信息"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", default=False, help="启用详细模式，显示更多信息"
    )
    parser.add_argument(
        "-d", "--dedup", action="store_true", default=False, help="启用自动去重功能，减少相似图片"
    )
    parser.add_argument(
        "-a",
        "--animation",
    ...
    parser.add_argument(
        "--dedup-threshold", type=float, default=0.98, help="去重相似度阈值，默认为0.98"
    )
    parser.add_argument(
        "--sort", action="store_true", default=False, help="根据内容对捕获的图片进行重新排序"
    )
    parser.add_argument(
        "--multi-level",
        action="store_true",
        default=False,
        help="启用多级处理流程（先粗略分析再精细分析）",
    )
    parser.add_argument("--coarse-interval", type=int, default=30, help="粗略分析的帧间隔")
    parser.add_argument("--coarse-threshold", type=float, default=0.95, help="粗略分析的相似度阈值")
    parser.add_argument("--coarse-method", type=int, default=2, help="粗略分析使用的算法")
    parser.add_argument("--parallel", action="store_true", default=False, help="启用并行处理")
    parser.add_argument(
        "--workers", type=int, default=None, help="并行处理的工作线程数，默认为CPU核心数"
    )
...
def mydo(
    ...
    # 初始化日志记录器
    logger = Logger(quiet=quiet, verbose_mode=verbose)  # 修改参数名以避免冲突
    ...
    if multi_level:
        logger.info("处理模式: 多级处理")
        logger.info(f"粗略分析帧间隔: {coarse_interval}")
        logger.info(f"粗略分析相似度阈值: {coarse_threshold}")
        logger.info(f"粗略分析算法: {get_method_name(coarse_method)}")
        logger.info(f"并行处理: {'启用' if parallel else '禁用'}")
        if parallel and workers:
            logger.info(f"并行工作线程数: {workers}")
    else:
        logger.info("处理模式: 标准处理")
    ...
    if multi_level:
        # 使用多级处理流程
        from pkg.multi_level_processing import MultiLevelProcessor
        ...
        # 处理视频
        captured_files = processor.process_video(VideoFilePath, outdir, spend)
            ...
            if isinstance(first_frame_data, tuple) and len(first_frame_data) == 2:
                ...
                for frame_data in ITERS:
                    if isinstance(frame_data, tuple) and len(frame_data) == 2:
                        ...
                        # 计算当前进度
                        progress = float(num_frames_new) / float(num_frames_all)
                        ...
                        if use_all and not quiet:
                            ...
                            all_results = runAllImageSimilaryFun(
                                frame, frame_new, isfile=False, isprint=False
                            )
                            if all_results:  # 确保结果不为None
                                logger.debug(
                                    "所有算法相似度: " + " ".join("%0.4f" % x for x in all_results)
                                )
                        ...
                        if result < Similarity:
                            ...
                            if animation > 0:
                                if potential_capture is None:
                                    ...
                                    # 计算与潜在捕获帧的相似度
                                    stability_result = comp(
                                        potential_capture, frame_new, method=method
                                    )
                                    stability_queue.append((stability_result, num_frames_new))

                                    if verbose:
                                        logger.verbose(
                                            f"  稳定性检测: 相似度 {stability_result:.6f} ({len(stability_queue)}/{animation})"
                                        )
                ...
                logger.info("无法获取第一帧，处理终止")
        except StopIteration:
            logger.info("处理过程中遇到意外结束")
    ...
    if dedup and len(captured_files) > 1:
        ...
        logger.info(
            f"去重完成! 保留 {len(deduplicated_files)} 张图片，删除了 {removed_count} 张相似图片"
        )
...
def dedup_images(image_files, outdir, method, threshold, logger):
    ...
    try:
        ...
        for i, (img_path, frame_num) in enumerate(image_files[1:], 1):
            logger.update_progress(
                i / total, total=total, current=i, prefix="去重进度: ", suffix=""
            )
...
def sort_images(image_files, outdir, method, logger):
    ...
    try:
        ...
        for i, (old_path, _) in enumerate(image_files):
            ...
            logger.update_progress(
                (i + 1) / total, total=total, current=i + 1, prefix="排序进度: ", suffix=""
            )
